  ...
  if (error) {
    return (
      <div className="nd-state nd-state--error">
        ...
        <p>{error}</p>
        <button className="btn btn-primary" onClick={() => navigate('/')}>← Back to Notes</button>
      </div>
    );
  }
  ...
  return (
    <div className="nd-wrapper">
      <article className="nd-article">
        <button className="nd-back" onClick={() => navigate('/')}>← Back</button>
        ...
        <header className="nd-header">
          ...
          <div className="nd-meta">
            ...
            <div className="nd-stats">
              ...
              <span>🕒 {new Date(note.updatedAt).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</span>
            </div>
          </div>
        </header>

        <div className="rendered-content nd-body" dangerouslySetInnerHTML={{ __html: note.html }} />
      </article>
    </div>
  );
}
